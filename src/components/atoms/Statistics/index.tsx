...
export const Statistics = () => {
  const [todayClick, setTodayClick] = useState<
    { click: any; location_id: any }[]
  >([]);
  ...
  const [topLocationToday, setTopLocationToday] = useState<{
    location_id: number;
    clicks: number;
  } | null>(null);
  ...
  useEffect(() => {
    let today = 0;
    const locationClicks: { [key: number]: number } = {};

    todayClick &&
      todayClick.forEach((c) => {
        today += c.click;
        // 부대별 클릭 수 집계
        if (c.location_id) {
          locationClicks[c.location_id] =
            (locationClicks[c.location_id] || 0) + c.click;
        }
      });
    ...
    for (const [locationId, clicks] of Object.entries(locationClicks)) {
      if (clicks > maxClicks) {
        maxClicks = clicks;
        topLocation = { location_id: parseInt(locationId), clicks };
      }
    }
    setTopLocationToday(topLocation);
  }, [todayClick]);

  const fetchStat = async () => {
    // 전체 클릭 수 조회 (모든 부대의 클릭 수 합계)
    let { data: fetchedTotalClick } = await supabase
      .from("clicks")
      .select("click");
    fetchedTotalClick && setTotalClick(fetchedTotalClick);

    // 오늘의 클릭 수 조회 (한국 시간 기준)
    let now = new Date();
    ...
    let { data: fetchedTodayClick } = await supabase
      .from("clicks")
      .select("id,created_at,click,location_id")
  ...
  return (
    ...
    >
      <div className="group-hover:hidden w-full h-full grid place-content-center">
        <svg
          className="w-6 h-6 group-hover:absolute group-hover:top-2 group-hover:left-2"
      ...
      <div className="hidden group-hover:block overflow-hidden">
        ...
        <ul className="w-full list-inside list-disc py-2">
          <li className="text-xs font-thin text-slate-50 break-all">
            Today: {todayClicked.toLocaleString()}
          </li>
          <li className="text-xs font-thin text-slate-50 break-all">
            Total: {totalClicked.toLocaleString()}
          </li>
          {topLocationToday && (
            <li className="text-xs font-thin text-slate-50 break-all">
              오늘 1위: 부대 {topLocationToday.location_id} (
              {topLocationToday.clicks.toLocaleString()}클릭)
            </li>
          )}
        </ul>
      </div>
