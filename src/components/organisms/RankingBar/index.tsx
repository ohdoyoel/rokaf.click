...
export const RankingBar = ({ locationId, score }: RankingBarProps) => {
  ...
  const fetchSortedLocations = async () => {
    try {
      ...
      // 점수 순으로 정렬
      const sorted = locationsWithScores.sort((a, b) => b.score - a.score);
  ...
  return (
    <div
      className="absolute inset-x-4 sm:inset-x-16 md:inset-x-28 bottom-0
                        h-16 transition-all ease-in-out duration-150 hover:h-4/5
                        group"
    >
      <BottomBar>
        <div
          className="group-hover:hidden h-full
                                grid grid-cols-3 gap-1 items-center"
        >
          <p className="col-span-3 sm:col-span-1 text-3xl text-center sm:text-left">
            🏅 리더보드
          </p>
